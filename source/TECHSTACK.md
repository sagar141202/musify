...
Musify uses a **client-server architecture** with a Python FastAPI backend acting as a smart proxy and intelligence layer, and a React Native (Expo) Android app as the frontend. Audio is never stored on the server — the backend fetches a direct YouTube CDN stream URL and hands it to the phone to play directly. This keeps server costs zero and latency minimal.
...
/storage/musify/
  ...
  musify.db            ← SQLite: offline metadata
```
...
```bash
# Backend .env
DATABASE_URL=postgresql+asyncpg://user:pass@host:5432/musify
